...
async function computeSurge() {
  ...
  todayOrders.forEach(o => {
    if (!userOrders[o.user]) userOrders[o.user] = [];
    userOrders[o.user].push(o);
  });
  ...
  for (const [user, orders] of Object.entries(userOrders)) {
    ...
    orders.forEach(o => {
      ...
      lines.forEach(line => {
        ...
        numbers.forEach(num => {
          orderCovered.add(num);
          numAmount[num] = (numAmount[num] || 0) + amtPerNum;
          totalAmount += amtPerNum;
        });
      });
      orderCovered.forEach(n => {
        numCount[n] = (numCount[n] || 0) + 1;
      });
    });
    const countTriggered = [];
    for (const [num, cnt] of Object.entries(numCount)) {
      if (totalOrders > 0 && cnt / totalOrders >= countThreshold) {
        countTriggered.push({ num, ratio: cnt / totalOrders });
      }
    }
    countTriggered.sort((a, b) => b.ratio - a.ratio);
    const amountTriggered = [];
    for (const [num, amt] of Object.entries(numAmount)) {
      if (totalAmount > 0 && amt / totalAmount >= amountThreshold) {
        amountTriggered.push({ num, ratio: amt / totalAmount });
      }
    }
...
function copyAllSurgeNums() {
  ...
  window._surgeResult.forEach(user => {
    user.countItems.forEach(i => allNums.add(i.num));
    user.amountItems.forEach(i => allNums.add(i.num));
  });
...
function updateCardA() {
  ...
  if (filterText) {
    ...
    tokens.forEach(token => {
      if (/^\d{1,2}$/.test(token)) {
        const n = token.padStart(2, '0');
        if (parseInt(n) >= 1 && parseInt(n) <= 49) targetNums.add(n);
      } else if (ZODIAC_NUMS[token]) {
        ZODIAC_NUMS[token].split(/[\s,，]+/).forEach(n => targetNums.add(n.padStart(2, '0')));
      } else if (D[token]) {
        const nums = keyToAllNums(token);
        nums.forEach(n => targetNums.add(n.padStart(2, '0')));
      }
    });
    if (targetNums.size > 0) {
      const negativeNums = [];
      for (const num of targetNums) {
        if (reportRiskData[num] !== undefined && reportRiskData[num] < 0) {
  ...
  if (!isNaN(nVal) && nVal > 0) {
    ...
    for (let i = 1; i <= 49; i++) {
      const n = i.toString().padStart(2, '0');
      if (!numberAmountCount[n]) entries.push({ num: n, cnt: 0 });
    }
  ...
  if (zodiacRankVisible) {
    ...
    zCountEntries.forEach((e, idx) => {
      html += `<span class="${zcm[e.zodiac] || ''}">${e.zodiac}</span>`;
      if (idx < zCountEntries.length - 1) html += '、';
    });
    html += '</div>';
    ...
    zAmtEntries.forEach((e, idx) => {
      html += `<span class="${zcm[e.zodiac] || ''}">${e.zodiac}</span>`;
      if (idx < zAmtEntries.length - 1) html += '、';
    });
    html += '</div>';
  }
...
function copyCardANumbers(type) {
  ...
  let lineIdx = 0;
  if (filterText) {
    if (type === 'risk') targetLineIndex = lineIdx;
    lineIdx++;
  }
  if (!isNaN(topNVal) && topNVal > 0) {
    if (type === 'active') targetLineIndex = lineIdx;
    lineIdx++;
    if (type === 'inactive') targetLineIndex = lineIdx;
    lineIdx++;
  }
