...
function showRecognizeModal() {
    ...
    <div class="modal-body" style="display:flex; flex-direction:column; gap:10px;">
      <div class="card recognize-card" style="flex:1; display:flex; flex-direction:column;">
        ...
        <div style="margin-top:10px;display:flex;gap:8px;flex-wrap:wrap;">
          ...
          <button class="btn btn-order-mark btn-sm" onclick="orderMarkSelection()" style="background:#8e44ad;color:#fff;">订单标记</button>
          ...
          <button class="btn btn-mark btn-sm" onclick="markSelection()">合并行</button>
  ...
  if (textarea) { const draftKey = `recognizeDraft_${currentRegion}`; const draft = localStorage.getItem(draftKey); if (draft) { textarea.value = draft; performRecognition(draft); } textarea.addEventListener('dragover', (e) => { e.preventDefault(); }); textarea.addEventListener('drop', (e) => { e.preventDefault(); const data = e.dataTransfer.getData('text/plain'); if (data) { textarea.value = data; performRecognition(data); showToast('已拖入文本'); } }); textarea.addEventListener('keydown', (e) => { if (e.altKey && e.key === 'w') { e.preventDefault(); orderMarkSelection(); } }); }
...
function orderMarkSelection() { const ta = document.querySelector('.source-order-input'); if (!ta) return; const start = ta.selectionStart, end = ta.selectionEnd; if (start === end) { showToast('请先选择文本'); return; } const selectedText = ta.value.substring(start, end); const stripped = selectedText.trim(); if (stripped.startsWith('--start--{') && stripped.endsWith('}--end--')) { showToast('已包含订单标记'); return; } const result = '--start--{\n' + selectedText + '\n}--end--'; ta.value = ta.value.substring(0, start) + result + ta.value.substring(end); ta.selectionStart = start; ta.selectionEnd = start + result.length; performRecognition(ta.value); }
    ...
    lines.forEach(line => {
      if(line.startsWith('特肖:')){
        ...
        if(match){
          ...
          } else if(playType.includes('连肖')||playType.includes('连尾')){
            // 连肖/连尾内容如"龙鸡-龙虎-鸡虎"，按-分割统计组数
            const groups = content.split('-').filter(i => i.trim());
...
async function showOrderRecord(filter='all'){ try{ const recs=await getOrderRecords(),users=getUsers(),today=getTodayCST(); const reports=await getReportOrderRecords(); const fd=document.getElementById('filterDate')?.value; const fRecs=fd?recs.filter(r=>r.date===fd):recs; const fReps=fd?reports.filter(r=>r.date===fd):reports; const uRecs = filter!=='all' ? fRecs.filter(r=>r.user===filter) : fRecs; const uReps = filter!=='all' ? fReps.filter(r=>r.user===filter) : fReps; const prizeInput = document.getElementById('prizeNumberInput'); if (prizeInput) window._savedPrizeFilter = prizeInput.value || ''; if(document.getElementById('orderWin'))document.getElementById('orderWin').remove(); const w=document.createElement('div'); w.className='floating-window'; w.id='orderWin'; w.style.width='750px'; w.style.height='600px'; w.style.left='50%'; w.style.top='50%'; w.style.transform='translate(-50%,-50%)'; let html=`<div class="modal-header"><h3>下单记录 <span style="font-size:12px;font-weight:normal;">(共${uRecs.length}单)</span></h3><div class="window-controls"><button onclick="maximizeWindow('orderWin')">🗖</button><button onclick="document.getElementById('orderWin').remove()">×</button></div></div><div class="modal-body" style="display:flex; flex-direction:column; height: calc(100% - 120px);">`;
  ...
  const filtered = filterVal ? uRecs.filter(r => orderContainsTarget(r.content, filterVal)) : uRecs;
  ...
  renderOrderStats(uRecs, uReps, filter, document.getElementById('prizeNumberInput')?.value?.trim());
  ...
  records.forEach(rec => {
    ...
    if (newMatch) {
      ...
      groups.forEach(group => {
        ...
        } else if (playType.includes('连肖')) {
          // 连肖数据格式: 龙鸡-龙虎-鸡虎 (组合字符串用-连接)
          const comboStrs = content.split('-');
          comboStrs.forEach(comboStr => {
            const chars = comboStr.split('');
            if (chars.length >= 2 && chars.length <= 5 && chars.every(c => ZODIAC_NUMS[c])) {
              const comboType = 'lianxiao' + chars.length;
              if (!stats[comboType]) stats[comboType] = { withYear: new Map(), withoutYear: new Map() };
              const hasYear = chars.some(c => c === curYearZodiac);
              (hasYear ? stats[comboType].withYear : stats[comboType].withoutYear).set(comboStr, ((hasYear ? stats[comboType].withYear : stats[comboType].withoutYear).get(comboStr) || 0) + amount);
              grandTotal += amount;
            }
          });
...
function buildHitDetail(hitByType) {
  const parts = [];
  const orderedTypes = ['特码','特肖','特肖本年肖','平特肖','平特肖带主肖','二连肖','二连肖带主肖','三连肖','三连肖带主肖','四连肖','四连肖带主肖','五连肖','五连肖带主肖','平特尾','平特尾零尾','二连尾','二连尾零尾','三连尾','三连尾零尾','四连尾','四连尾零尾','五连尾','五连尾零尾','五不中','六不中','七不中','八不中','九不中','十不中','十一不中','十二不中','十三不中','十四不中','十五不中','十六不中','十七不中','十八不中','二中二','三中二','三中三','平码','特碰'];
...
function processComboLineDuijiangNew(stats, playType, content, amt, drawZodiacsSet, drawNumbersSet, drawNumbersZhengma, hasValidDraw) {
  ...
  combos.forEach(combo => {
    ...
    if (comboType === '平特肖') {
      ...
      hasYearZodiac = tokens.some(t => [...t].some(c => c === curYearZodiac));
    ...
    // 连肖/连尾也是每个组合独立一注，计入effectiveCount
    const isPerItem = ['平特肖','平特肖带主肖','平特尾','平特尾零尾','平码','二连肖','二连肖带主肖','三连肖','三连肖带主肖','四连肖','四连肖带主肖','五连肖','五连肖带主肖','二连尾','二连尾零尾','三连尾','三连尾零尾','四连尾','四连尾零尾','五连尾','五连尾零尾'].includes(comboType);
    ...
    if (hasValidDraw) {
      ...
      else if (['二连肖','二连肖带主肖','三连肖','三连肖带主肖','四连肖','四连肖带主肖','五连肖','五连肖带主肖'].includes(comboType)) {
        // 连肖组合如"龙鸡"拆成单个生肖，检查所有生肖是否都在开奖中
        hit = tokens.some(t => {
          const chars = [...t];
          return chars.length >= 2 && chars.every(c => drawZodiacsSet.has(c));
        });
      }
      else if (['二连尾','二连尾零尾','三连尾','三连尾零尾','四连尾','四连尾零尾','五连尾','五连尾零尾'].includes(comboType)) { hit = tokens.some(t => { const d=t.replace('尾',''); for(let i=0;i<=4;i++){ const n=(i*10+parseInt(d)).toString().padStart(2,'0'); if(drawNumbersSet.has(n)) return true; } return false; }); }
      else if (['五不中','六不中','七不中','八不中','九不中','十不中','十一不中','十二不中','十三不中','十四不中','十五不中','十六不中','十七不中','十八不中'].includes(comboType)) { hit = !tokens.some(t => drawNumbersSet.has(t.padStart(2,'0'))); }
      else if (comboType === '二中二' || comboType === '三中三') { hit = tokens.every(t => drawNumbersZhengma.includes(t.padStart(2,'0'))); }
      else if (comboType === '三中二') { const zhengmaHit = tokens.filter(t => drawNumbersZhengma.includes(t.padStart(2,'0'))).length; hit = zhengmaHit >= 2; }
      if (hit) {
        if (isPerItem) {
          let hitCount = 0;
          if (comboType === '平特肖' || comboType === '平特肖带主肖') { hitCount = tokens.filter(t => drawZodiacsSet.has(t)).length; }
            ...
            hitCount = tokens.filter(t => {
              const chars = [...t];
              return chars.length >= 2 && chars.every(c => drawZodiacsSet.has(c));
            }).length;
          }
          else if (['二连尾','二连尾零尾','三连尾','三连尾零尾','四连尾','四连尾零尾','五连尾','五连尾零尾'].includes(comboType)) {
            hitCount = tokens.filter(t => {
              const d = t.replace('尾','');
              for(let i=0;i<=4;i++){ const n=(i*10+parseInt(d)).toString().padStart(2,'0'); if(drawNumbersSet.has(n)) return true; }
              return false;
            }).length;
          }
...
function processComboLineDuijiangOld(stats, match, drawZodiacsSet, drawNumbersSet, drawNumbersZhengma, drawTeMa, hasValidDraw) {
  ...
  combos.forEach(combo => {
    ...
    if (tokens.length === 1) {
      ...
      else if (tokens.length === 3) { comboType = '三中二'; }
      else { const bzMap = {5:'五不中',6:'六不中',7:'七不中',8:'八不中',9:'九不中',10:'十不中',11:'十一不中',12:'十二不中',13:'十三不中',14:'十四不中',15:'十五不中',16:'十六不中',17:'十七不中',18:'十八不中'}; comboType = bzMap[tokens.length] || '五不中'; }
    }
    if (!comboType) return;
...
function removeSeparators() { const ta = document.querySelector('.source-order-input'); if(!ta) return; const s=ta.selectionStart,e=ta.selectionEnd; if(s===e){showToast('请先选择文本');return;} const sel=ta.value.substring(s,e); const cleaned=sel.replace(/[^\dA-Za-z\u4e00-\u9fa5]|一|\s/g,''); ta.value=ta.value.substring(0,s)+cleaned+ta.value.substring(e); performRecognition(ta.value); }

function replaceSeparators() { const ta=document.querySelector('.source-order-input'); if(!ta) return; const s=ta.selectionStart,e=ta.selectionEnd; if(s===e){showToast('请先选择文本');return;} const sel=ta.value.substring(s,e); const replaced=sel.replace(/[^\dA-Za-z\u4e00-\u9fa5\s]/g,'-'); ta.value=ta.value.substring(0,s)+replaced+ta.value.substring(e); performRecognition(ta.value); }
...
function decomposeKeys(text) {
  /* 贪婪分解文本为字典键列表（从左到右匹配最长键）。
   * 如 '红蓝波大' → ['红', '蓝波', '大']。
   */
  const keys = [];
  let i = 0;
  while (i < text.length) {
    ...
    for (let end = text.length; end > i; end--) {
      const sub = text.substring(i, end);
      if (D[sub] !== undefined) {
        keys.push(sub);
        i = end;
        found = true;
...
function semanticReplace() {
  ...
  // 按"的"分割成段，每段内分解为字典键
  const segments = sel.split('的').map(s => s.trim()).filter(s => s);
  if (!segments.length) { showToast('未识别有效分类'); return; }

  // 每段取并集，段间取交集
  let result = null;
  for (const seg of segments) {
    const keys = decomposeKeys(seg);
    if (!keys.length) { showToast('未识别有效分类'); return; }
    // 段内取并集
    const union = new Set();
    for (const k of keys) {
      const nums = keyToAllNums(k);
      nums.forEach(n => union.add(n));
    }
    if (!union.size) { showToast('无共同号码'); return; }
    if (result === null) {
      result = union;
    } else {
      result = new Set([...result].filter(x => union.has(x)));  // 段间取交集
    }
    if (!result.size) { showToast('无共同号码'); return; }
  }

  const sortedNums = [...result].sort((a, b) => parseInt(a) - parseInt(b));
