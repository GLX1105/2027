...
function markRegion(region) {
  ...
  const markedLines = lines.map(line => {
    ...
    for (const p of allPrefixesSorted) {
      if (trimmed.startsWith(p)) { trimmed = trimmed.substring(p.length).trim(); break; }
    }
...
function renderOrderStats(allOrders, allReports, filterUser, prizeNum) {
  ...
  const mul = parseFloat(document.getElementById('multipleVal')?.value) || 1; const rr = parseFloat(document.getElementById('rebateRate')?.value) || 0;
  ...
  if (prizeNum) {
    ...
    allOrders.forEach(it => { totalSpecial += getSpecialAmountFromOrder(it.content, prizeNum); if (orderContainsTarget(it.content, prizeNum)) hitCount++; });
    allReports.forEach(it => { reportSpecial += getSpecialAmountFromOrder(it.content, prizeNum); });
  }
  ...
  let html = '<div class="stats-block">'; html += '<div class="stats-row">';
