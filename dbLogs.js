// ===== dbLogs.js - 操作日志的写入、读取、分页、清理与窗口展示 =====
  ...
  try {
    ...
    allReq.onsuccess = () => {
      ...
      if (allLogs.length > 1000) {
        ...
        sorted.forEach(log => {
          if (log.timestamp.slice(0, 10) === earliestDate) store.delete(log.id);
        });
      }
    };
  } catch (e) {}
...
async function updateLogCount() {
  ...
  try {
    ...
    span.textContent = logs.length;
  } catch (e) { span.textContent = '0'; }
}
...
function renderLogPage() {
  ...
  const pageSize = window._logPageSize;
  const start = 0;
  ...
  const actionColors = {
    'save_order':'log-type-save','save_report':'log-type-save','delete_order':'log-type-delete',
    'delete_report':'log-type-delete','restore':'log-type-restore','permanent_delete':'log-type-delete',
    'reset':'log-type-reset','export':'log-type-export','import':'log-type-export',
    'switch':'log-type-switch','login':'log-type-login','logout':'log-type-login'
  };
  let html = '';
...
function loadMoreLogs() {
  ...
  if (dateFilter) filteredLogs = filteredLogs.filter(log => log.timestamp.slice(0, 10) === dateFilter);
  const pageSize = window._logPageSize;
  const currentPage = window._logPage;
