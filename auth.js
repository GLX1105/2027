...
function generateSelfVerifyingCard(expireDays) {
  const now = Date.now(); const expireMs = expireDays * 86400000;
...
function verifySelfVerifyingCard(code) {
  ...
  try { createTime = parseInt(parts[0], 36); expireMs = parseInt(parts[1], 36); }
  catch (e) { return { valid: false, reason: '卡密无效' }; }
...
function checkCurrentAccess() {
  ...
  if (session.role === 'card' && session.cardCode) {
    ...
    if (!result.valid) { clearAuthSession(); return false; }
    return true;
  }
  return false;
}
...
function showLoginScreen() {
  ...
  overlay.innerHTML = `<div class="login-box"><h3>🔐 系统登录</h3><input type="password" id="loginPassword" placeholder="管理员密码"><button class="btn" style="background:#2c3e50;color:#fff;" id="adminLoginBtn">管理员登录</button><hr style="margin:15px 0;"><input type="text" id="cardCodeInput" placeholder="卡密"><button class="btn" style="background:#e67e22;color:#fff;" id="cardLoginBtn">卡密登录</button><div class="link" id="switchToCard">使用卡密登录</div><div class="link" id="switchToAdmin" style="display:none;">管理员登录</div></div>`;
  ...
  document.getElementById('adminLoginBtn').onclick = () => {
    ...
    if (pwd === ADMIN_PASSWORD) { setAuthSession({ role: 'admin' }); overlay.remove(); initMainSystem(); }
    else showToast('管理员密码错误');
  };
  document.getElementById('cardLoginBtn').onclick = () => {
    ...
    if (result.valid) { setAuthSession({ role: 'card', cardCode: code }); overlay.remove(); initMainSystem(); }
