// ===== 新版字典与识别核心 =====
const PLAY_NAMES = ['连肖', '连尾', '二中二', '三中二', '三中三', '特碰', '不中', '平特肖', '平特尾', '特肖', '特码', '平码'];
...
function toNum(s) { if (!s) return 0; s = String(s).trim(); if (/^-?\d+(\.\d+)?$/.test(s)) return parseFloat(s) % 1 === 0 ? parseInt(s) : parseFloat(s); const mDu = s.match(/^(\d+(?:\.\d+)?)(万|千|仟|佰|百|拾|十)$/); if (mDu) { const mul = { '万': 10000, '千': 1000, '仟': 1000, '百': 100, '佰': 100, '十': 10, '拾': 10 }; return parseFloat(mDu[1]) * mul[mDu[2]]; } const m = { 零: 0, 〇: 0, 一: 1, 壹: 1, 二: 2, 贰: 2, 两: 2, 三: 3, 叁: 3, 四: 4, 肆: 4, 五: 5, 伍: 5, 六: 6, 陆: 6, 七: 7, 柒: 7, 八: 8, 捌: 8, 九: 9, 玖: 9 }; const u = { 十: 10, 拾: 10, 百: 100, 佰: 100, 千: 1000, 仟: 1000, 万: 10000 }; let r = 0, c = 0, t = 0; for (let i = 0; i < s.length; i++) { const ch = s[i]; if (m[ch] !== undefined) { t = m[ch]; } else if (u[ch] !== undefined) { const ut = u[ch]; if (t === 0 && (ch == '十' || ch == '拾')) t = 1; if (ut >= 10000) { c = (c + t) * ut; r += c; c = 0; } else { c += t * ut; } t = 0; } } r += c + t; const tm = s.match(/([百佰千仟万亿])([一二三四五六七八九两])$/); if (tm) { const uv = u[tm[1]] || 0; const dv = m[tm[2]] || 0; if (uv > 1 && dv > 0) r += dv * Math.floor(uv / 10) - dv; } return r || 0; }
...
function tailCKeepOrder(tStr, k) { const ns = tStr.split(/[,\-，]/).filter(n => n.trim()); return combosNoSort(ns, k).map(c => c.join('')); }
...
const AMT_RE_STR = '(\\d+万?|[一二三四五六七八九十百千两]+)';
const SEP_CHARS = '[\\s\\-,\\.\u3001\uff0c\u3002\uff01\uff1f\uff1b\uff1a\\(\\)\\[\\]{}<>/|@#$%^&*+=]';
const SEP = SEP_CHARS + '*';
